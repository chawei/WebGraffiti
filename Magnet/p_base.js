...
Processing.prototype.drawFreehandVertex = function(x1, y1, x2, y2) {
...
Processing.prototype.drawFreehandArcVetex = function(x, y, w, h, start, angle, isClockWise) {
  ...
  var totalDots = Math.floor( (w+h)*angle/100 );
	if(totalDots<20)
    totalDots = 20;
...
Processing.prototype.drawFreehandArc = function(x, y, w, h, start, end) {
  ...
  for(var i=0; i<totalDots; i++) {
		if(currentAngle>=start && currentAngle<=end) {
    	var p = new Point( x+Math.cos(currentAngle)*w/2*(1+Math.random()*freeFactor), y+Math.sin(currentAngle)*h/2*(1+Math.random()*freeFactor) );
			dots.add(p);
    }
		currentAngle+=angleGap;
  }
